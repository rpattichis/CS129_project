...
X_Train = zeros(numFreq, numCoeff+1, numChannels, numTrain);
X_Val = zeros(numFreq, numCoeff+1, numChannels, numVal);
X_Test = zeros(numFreq, numCoeff+1, numChannels, numTest);

for i =1:numTrain
    X_Train(:,:,i) = reshape(X_tr(:,i), [numFreq, numCoeff+1, numChannels]);
end
for i =1:numVal
    X_Val(:,:,i) = reshape(X_val(:,i), [numFreq, numCoeff+1, numChannels]);
end

for i =1:numTest
    X_Test(:,:,i) = reshape(X_test(:,i), [numFreq, numCoeff+1, numChannels]);
end

%%
num_classes = length(genres);
layers = [imageInputLayer([numFreq, numCoeff+1, 1])
